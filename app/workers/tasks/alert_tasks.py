...
async def dispatch_company_webhook(session, company_id: uuid.UUID, event_type: str, payload: dict) -> None:
    ...
    async def _check():
        ...
        async with AsyncSessionLocal() as session:
            ...
            for cert in certificates:
                ...
                await dispatch_company_webhook(session, cert.company_id, "certificate.expiring", webhook_payload)
                
                logger.info(f"Certificate expiry alert sent: cert={cert.id} company={cert.company_id} days_left={days_left}")
                alerts_triggered += 1
        ...
        async with AsyncSessionLocal() as session:
            ...
            for caf in cafs:
                ...
                if pct_used >= 80.0:
                    ...
                    # Trigger email
                    for email in emails:
                        subject = f"ALERTA: Folios CAF tipo {caf.dte_type} agotados al {pct_used:.1f}%"
                        html = f"""
                        <html><body>
                    ...
                    await dispatch_company_webhook(session, caf.company_id, "caf.depleted", webhook_payload)
                    
                    logger.info(f"CAF depletion alert sent: caf={caf.id} company={caf.company_id} pct_used={pct_used:.1f}%")
                    alerts_triggered += 1
