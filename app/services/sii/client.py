...
class SIIWebServicesClient:
    ...
    def __init__(self, environment: str = "certification"):
        self.env_urls = SIIEnvironment.PRODUCTION if environment == "production" else SIIEnvironment.CERTIFICATION
    ...
    async def _check_simulated_failure(self) -> None:
        if self.settings.SII_SIMULATE_FAILURE:
            ...
            raise httpx.HTTPStatusError(
                ...
                response=httpx.Response(500, request=httpx.Request("POST", "https://api.sii.example.local"))
            )
   ...
   <soapenv:Body>
    ...
    async def enviar_dte(self, signed_dte_xml: bytes, token: str, rut_emisor: str, rut_empresa: str) -> str:
        """
