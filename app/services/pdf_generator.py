...
class PDFGenerator:
    ...
    def generate_dte_pdf(dte_data: dict, ted_xml_string: str) -> bytes:
        ...
        c.drawString(
            40,
            height - 85,
            f"Dirección: {dte_data.get('emisor_address', 'Calle Falsa 123, Santiago')}",
        )
        ...
        c.drawCentredString(
            width - 120, height - 60, f"R.U.T.: {dte_data.get('emisor_rut', '11111111-1')}"
        )
        c.drawCentredString(
            width - 120,
            height - 75,
            "FACTURA ELECTRÓNICA" if dte_data.get("dte_type") == 33 else "DOCUMENTO ELECTRÓNICO",
        )
        ...
        c.setFont("Helvetica", 10)
        c.drawString(
            40, height - 130, f"Señor(es): {dte_data.get('receptor_name', 'Cliente Ficticio')}"
        )
        c.drawString(40, height - 145, f"R.U.T.: {dte_data.get('receptor_rut', '22222222-2')}")
        c.drawString(
            40,
            height - 160,
            f"Fecha Emisión: {dte_data.get('issue_date', datetime.now().strftime('%Y-%m-%d'))}",
        )
