...
class CRUDCertificate:
    async def get(self, db: AsyncSession, id: uuid.UUID) -> Optional[Certificate]:
    ...
    async def get_active_for_company(self, db: AsyncSession, company_id: uuid.UUID) -> Optional[Certificate]:
        result = await db.execute(
            select(Certificate)
            .where(Certificate.company_id == company_id, Certificate.is_active == True)
        )
        return result.scalars().first()

    async def create(self, db: AsyncSession, *, obj_in: CertificateCreate, company_id: uuid.UUID) -> Certificate:
