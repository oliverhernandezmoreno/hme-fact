...
class SubscriptionPlan(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    features: Mapped["SubscriptionFeature"] = relationship(
        "SubscriptionFeature", back_populates="plan", uselist=False
    )
...
class SubscriptionFeature(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    plan_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("subscription_plans.id", ondelete="CASCADE"), unique=True
    )
    ...
    support_level: Mapped[str] = mapped_column(
        String(50), default="community", nullable=False
...
class Subscription(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    plan: Mapped["SubscriptionPlan"] = relationship(
        "SubscriptionPlan", foreign_keys=[plan_id], back_populates="subscriptions"
    )
    previous_plan: Mapped["SubscriptionPlan | None"] = relationship(
        "SubscriptionPlan", foreign_keys=[upgraded_from_plan_id]
    )
...
class UsageMetric(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "company_id", "period_month", "period_year", name="uix_usage_metrics_company_period"
        ),
    )
...
class BillingEvent(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    )  # activation, renewal, upgrade, downgrade, cancellation, trial_start, trial_end
    description: Mapped[str | None] = mapped_column(Text)
