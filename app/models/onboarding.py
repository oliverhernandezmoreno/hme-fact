...
class OnboardingStepDefinition(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("workflow_id", "code", name="uq_step_def_workflow_code"),)

    workflow_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("onboarding_workflows.id", ondelete="CASCADE")
    )
    ...
    workflow: Mapped["OnboardingWorkflow"] = relationship(
        "OnboardingWorkflow", back_populates="step_definitions"
    )
...
class OnboardingSession(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    workflow_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("onboarding_workflows.id")
    )
    current_step_code: Mapped[str | None] = mapped_column(String(100))
    status: Mapped[str] = mapped_column(
        String(50), default="not_started", nullable=False
    )  # not_started, in_progress, blocked, completed, abandoned, skipped
    ...
    step_statuses: Mapped[list["OnboardingStepStatus"]] = relationship(
        "OnboardingStepStatus", back_populates="session", cascade="all, delete-orphan"
    )
...
class OnboardingStepStatus(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    step_definition_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("onboarding_step_definitions.id")
    )
    step_code: Mapped[str] = mapped_column(String(100), nullable=False)
    status: Mapped[str] = mapped_column(
        String(50), default="pending", nullable=False
    )  # pending, available, in_progress, completed, skipped, blocked, failed
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    session: Mapped["OnboardingSession"] = relationship(
        "OnboardingSession", back_populates="step_statuses"
    )
