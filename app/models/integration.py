...
class IntegrationConnection(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    provider: Mapped[str] = mapped_column(String, nullable=False, index=True) # shopify, woocommerce, mercadolibre, pos, erp
...
class IntegrationEvent(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    connection_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("integration_connections.id", ondelete="CASCADE"), nullable=True, index=True
    )
    event_type: Mapped[str] = mapped_column(String, nullable=False) # order_created, invoice_sync
    status: Mapped[str] = mapped_column(String, nullable=False, index=True) # pending, success, failed
...
class WebhookSubscription(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    event_types: Mapped[dict] = mapped_column(JSONB, nullable=False, default=[]) # list of events like dte.issued
