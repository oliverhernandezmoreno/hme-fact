...
from app.schemas.caf import CAFFIleUploadResponse, CAFFileResponse
...
async def upload_caf_file(
    *,
    ...
    if company.rut and company.rut.replace(".", "").replace("-", "") != parsed_data["rut_emisor"].replace(".", "").replace("-", ""):
        raise HTTPException(
            status_code=400, 
    ...
    if existing.scalars().first():
        raise HTTPException(status_code=409, detail="Este rango de folios (CAF) ya fue subido anteriormente.")
...
async def list_caf_files(
    ...
    stmt = select(CAFFile).where(CAFFile.company_id == company_id).order_by(CAFFile.authorization_date.desc())
