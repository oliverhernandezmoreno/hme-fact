...
from app.workers.tasks.alert_tasks import check_expiring_certificates_task, check_depleted_cafs_task
...
def test_check_expiring_certificates(mock_send_outbound_webhook, mock_email_service_class, mock_session_maker):
    ...
    mock_email_service.send_simple_email.assert_called_once_with(
        "admin@company.com",
        ANY,
        ANY
    )
...
@patch("app.modules.notifications.services.email_service.EmailService")
@patch("app.workers.integration_worker.send_outbound_webhook")
def test_check_depleted_cafs(mock_send_outbound_webhook, mock_email_service_class, mock_session_maker):
    mock_session = AsyncMock()
    mock_session_maker.return_value.__aenter__.return_value = mock_session
