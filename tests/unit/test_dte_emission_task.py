...
async def test_process_dte_emission_async(
    ...
    # Mock Decryption
    with patch("app.services.certificate_security.CertificateSecurityService.decrypt_data") as mock_decrypt:
...
async def test_process_dte_emission_contingency_on_circuit_breaker_open(
    ...
    with patch("app.services.certificate_security.CertificateSecurityService.decrypt_data") as mock_decrypt:
        ...
        mock_client_instance = mock_sii_client_class.return_value
        mock_client_instance.get_token = AsyncMock(side_effect=SIICircuitBreakerOpenException("Circuit open"))
