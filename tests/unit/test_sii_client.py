...
async def test_get_token_misses_cache_calls_sii_and_caches_result():
    ...
    with patch("redis.asyncio.from_url", return_value=mock_redis), \
         patch.object(client, "_fetch_token_from_sii", AsyncMock(return_value="NEW_TOKEN_999")) as mock_fetch:
...
async def test_get_token_redis_error_falls_back_to_direct_sii_call():
    ...
    with patch("redis.asyncio.from_url", side_effect=Exception("Redis connection error")), \
         patch.object(client, "_fetch_token_from_sii", AsyncMock(return_value="DIRECT_TOKEN_555")) as mock_fetch:
