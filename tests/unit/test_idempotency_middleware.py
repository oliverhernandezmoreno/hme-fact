    ...
    async with AsyncClient(
        ...
        with patch("redis.asyncio.from_url", return_value=mock_redis):
            ...
            response1 = await client.post(
                "/public/resource", headers={"idempotency-key": "unique-key-123"}
            )
            ...
            mock_redis.set.assert_any_call(
                "idempotency:global:unique-key-123", "processing", nx=True, ex=30
            )
    ...
    async with AsyncClient(
        ...
        with patch("redis.asyncio.from_url", return_value=mock_redis):
            response = await client.post(
                "/public/resource", headers={"idempotency-key": "concurrent-key"}
            )
...
async def test_idempotency_fail_open_on_redis_error(dummy_app):
    async with AsyncClient(
        ...
        with patch("redis.asyncio.from_url", side_effect=Exception("Redis down")):
            response = await client.post(
                "/public/resource", headers={"idempotency-key": "redis-down-key"}
            )
