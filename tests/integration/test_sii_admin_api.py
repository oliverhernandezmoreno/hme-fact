...
async def test_sii_status_forbidden_for_regular_user(client: AsyncClient, auth_headers: dict) -> None:
    # Regular user is not superadmin
    with patch("app.modules.rbac.services.rbac_service.RBACService.is_super_admin", new_callable=AsyncMock) as mock_check:
...
async def test_sii_clear_failure_simulation_success(client: AsyncClient, auth_headers: dict) -> None:
    with patch("app.modules.rbac.services.rbac_service.RBACService.is_super_admin", new_callable=AsyncMock) as mock_check, \
         patch("app.api.v1.endpoints.sii_admin.redis.from_url") as mock_redis_class:
        ...
        response = await client.post("/api/v1/admin/sii/clear-failure-simulation", headers=auth_headers)
        assert response.status_code == 200
